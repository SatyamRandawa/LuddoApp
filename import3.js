const {getDishNum } = require('./helper/game_fun_helper');
 ...
 module.exports = {
	...
	dice: function () {
		console.log( "G start time import3 js file call line no - 4"); 
	...
	gameLogic: async function (game, playerId, pos, chipsToMove) {
		...
		if (game.waitingForMove) {
			...
			var allOnStart = true;
			for (var i = 0; i < 1;i++) if (game.players[game.playerTurn].chips[i].distance > 0 && game.players[game.playerTurn].chips[i].distance != 59) allOnStart = false;
	...
	createGame: function(players, gameSettings) {
		...
		game.throwsLeft=1;
		game.waitingForMove = false;
		game.nextDice= dice(gameIdIncrement);
		game.lastDice=3;
		game.posiblePos=[];
		game.posiblePosDest=[];
...
async function dice(G_id) {
    try {

         let ddss =   await getDishNum(G_id);
